...
int64_t  leg_peak_start_time[4]={0,0,0,0}; //to record when a peak starts
int64_t  leg_peak_end_time[4]={0,0,0,0}; //to record when a peak starts
float max_peak_current[4]={0,0,0,0};
float last_peak_current[4]={0,0,0,0};
 
float gait[4]={0,0,0.5,0.5};
float buffer_gait[4]={0,0,0.5,0.5};
int64_t  gait_update_time[4]={0,0,0,0};
int gait_update_flag[4]={0,0,0,0};

void adjust_gait(int leg,int64_t  t){
    float contact_position;
    float local_rule_step=0.1;
    
    if (leg==0 || leg==3){
      if (leg==0)
        odrive_right_serial << "r axis" << 0 << ".encoder.pos_estimate\n";
      else
        odrive_right_serial << "r axis" << 1 << ".encoder.pos_estimate\n";

      contact_position=fmod(odrive_right.readFloat(),1.0);  
    
    }
    else{
      if (leg==1)
        odrive_left_serial << "r axis" << 0 << ".encoder.pos_estimate\n";
      else
        odrive_left_serial << "r axis" << 1 << ".encoder.pos_estimate\n";

      contact_position=fmod(odrive_left.readFloat(),1.0);  
    
    }

    
    if (contact_position<0.5)
      buffer_gait[leg]=gait[leg]+local_rule_step;
    else
      buffer_gait[leg]=gait[leg]-local_rule_step;
      
    gait_update_time[leg]=t;
    gait_update_flag[leg]=1;
         
    return;
}
void peak_detection(int leg, int64_t  t){
  int axis;
  float peak_threshold=4; //in A
  float velocity_threshold=0.5; //in turn/s
  ...
  if (t-gait_update_time[leg]>500 && gait_update_flag[leg]==1){
      gait_update_flag[leg]=0;
      gait[leg]=buffer_gait[leg];
  }
    ...
    if (c == 'c') {
      ...
      while(true){
        ...
        if (current_time-last_time >time_interval){
            odrive_left.SetPosition(0, 1.05-(pos_m0+gait[1]));
            odrive_left.SetPosition(1, 1.05-(pos_m1+gait[2]));
          
            odrive_right.SetPosition(0, pos_m0+gait[0]);
            odrive_right.SetPosition(1, pos_m1+gait[3]);
